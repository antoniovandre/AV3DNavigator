 ...
 * 
 * Última atualização: 16-08-2025. Não considerando alterações em variáveis globais.
 */
				...
				if (ContadorFrames == FramesDeslocamento)
					...
					switch (keyCode)
						...
						case KeyEvent.VK_UP:
							...
							if (ke.isShiftDown())
								{
								if ((Math.abs(x + Math.cos((Phi + Math.PI / 2) * Math.cos(Rot) * Math.cos(Phi)) * Math.cos((Teta + Math.PI / 2) * Math.sin(Rot)) * (Math.cos(Teta) >= 0 ? (Math.sin(Phi) >= 0 ? -1 : 1) : 1) * (Math.sin(Rot) >= 0 ? 1 : -1)) >= AntonioVandre.MaximoValorReal) || (Math.abs(y - Math.cos((Phi + Math.PI / 2) * Math.cos(Rot) * Math.cos(Phi)) * Math.sin((Teta + Math.PI / 2) * Math.sin(Rot)) * (Math.cos(Teta) >= 0 ? 1 : (Math.sin(Phi) >= 0 ? (Math.cos(Rot) * Math.sin(Rot) >= 0 ? -1 : 1) : (Math.cos(Rot) * Math.sin(Rot) >= 0 ? 1 : -1)))) >= AntonioVandre.MaximoValorReal) || (Math.abs(z - Math.sin((Phi + Math.PI / 2) * Math.cos(Rot) * Math.cos(Phi)) * (Math.cos(Phi) >= 0 ? 1 : (Math.sin(Phi) * Math.cos(Phi) >= 0 ? -1 : 1))) >= AntonioVandre.MaximoValorReal))
									VariavelLimiteAtingido();
								else
									{
									x += Math.cos((Phi + Math.PI / 2) * Math.cos(Rot) * Math.cos(Phi)) * Math.cos((Teta + Math.PI / 2) * Math.sin(Rot)) * (Math.cos(Teta) >= 0 ? (Math.sin(Phi) >= 0 ? -1 : 1) : 1) * (Math.sin(Rot) >= 0 ? 1 : -1);
									y -= Math.cos((Phi + Math.PI / 2) * Math.cos(Rot) * Math.cos(Phi)) * Math.sin((Teta + Math.PI / 2) * Math.sin(Rot)) * (Math.cos(Teta) >= 0 ? 1 : (Math.sin(Phi) >= 0 ? (Math.cos(Rot) * Math.sin(Rot) >= 0 ? -1 : 1) : (Math.cos(Rot) * Math.sin(Rot) >= 0 ? 1 : -1)));
