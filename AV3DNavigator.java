 ...
 * 
 * Última atualização: 05-08-2025. Não considerando alterações em variáveis globais.
 */
					...
					switch (keyCode)
						...
						case KeyEvent.VK_K:
							if (ke.isControlDown())
								...
								if (Math.abs(Teta) < TetaMax - DeslocamentoAngular) {if (Math.abs(Phi) < PhiMax - DeslocamentoAngular) {if ((! (Math.abs(Teta + DeslocamentoAngular * Math.cos(Rot + Phi0Rotacao)) >= AntonioVandre.MaximoValorReal)) || (! (Math.abs(Phi - DeslocamentoAngular * Math.sin(Rot + Teta0Rotacao) * Math.cos(Phi)) >= AntonioVandre.MaximoValorReal)) || (! (Math.abs(Rot + DeslocamentoAngular * Math.cos(Rot) * Math.sin(Phi)) >= AntonioVandre.MaximoValorReal))) {Teta += DeslocamentoAngular * Math.cos(Rot + Phi0Rotacao); Phi -= DeslocamentoAngular * Math.sin(Rot + Teta0Rotacao) * Math.cos(Phi); Rot += DeslocamentoAngular * Math.cos(Rot) * Math.sin(Phi); x = xRotacao + RaioRot * Math.cos(Phi + Math.PI) * Math.cos(Teta); y = yRotacao - RaioRot * Math.cos(Phi + Math.PI) * Math.sin(Teta); z = zRotacao - RaioRot * Math.sin(Phi + Math.PI); ContadorFrames = 0;} else VariavelLimiteAtingido();} else {Phi -= Math.signum(Phi) * DeslocamentoAngular; ContadorFrames = FramesDeslocamento; Phit = Phi; FlagPhiInferior = 1;}} else {Teta -= Math.signum(Teta) * DeslocamentoAngular; ContadorFrames = FramesDeslocamento; Tetat = Teta; FlagTetaInferior = 1;}
								}
						...
						case KeyEvent.VK_UP:
							...
							if (ke.isShiftDown())
								{
								if ((Math.abs(x - Math.cos((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot)) * Math.cos((Phi - Math.PI / 2) * Math.sin(Rot) + Teta * Math.cos(Rot))) >= AntonioVandre.MaximoValorReal) || (Math.abs(y + Math.cos((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot)) * Math.sin((Phi - Math.PI / 2) * Math.sin(Rot) + Teta * Math.cos(Rot))) >= AntonioVandre.MaximoValorReal) || (Math.abs(z + Math.sin((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot))) >= AntonioVandre.MaximoValorReal))
									VariavelLimiteAtingido();
								else
									{
									x -= Math.cos((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot)) * Math.cos((Phi - Math.PI / 2) * Math.sin(Rot) + Teta * Math.cos(Rot));
									y += Math.cos((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot)) * Math.sin((Phi - Math.PI / 2) * Math.sin(Rot) + Teta * Math.cos(Rot));
									z += Math.sin((Phi - Math.PI / 2) * Math.cos(Rot) + Teta * Math.sin(Rot));
									}
								}
							else
								{
						...
						case KeyEvent.VK_LEFT:
							...
							if ((Math.abs(x - Math.cos(Phi + Rot) * Math.cos(Teta - Math.PI / 2 + Rot)) >= AntonioVandre.MaximoValorReal) || (Math.abs(y + Math.cos(Phi + Rot) * Math.sin(Teta - Math.PI / 2 + Rot)) >= AntonioVandre.MaximoValorReal) || (Math.abs(z + Math.sin(Rot)) >= AntonioVandre.MaximoValorReal))
								VariavelLimiteAtingido();
							else
								{
								x -= Math.cos(Phi + Rot) * Math.cos(Teta - Math.PI / 2 + Rot);
								y += Math.cos(Phi + Rot) * Math.sin(Teta - Math.PI / 2 + Rot);
								z += Math.sin(Rot);
								}
