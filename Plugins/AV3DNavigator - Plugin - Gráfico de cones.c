...
Arquivo gerador de um espaço do AV3DNavigator gráfico de cones tridimensional.
...
int main (int argc, char * argv[])
	...
	for (i = 0; i < argi; i++)
		for (j = 0; j < resolucao; j++)
			...
			printf("%Lf,%Lf,0;%Lf,%Lf,0;0,%Lf,%Lfc%s|", raio * sinl(2 * M_PI * j / resolucao), espacamento * i + raio * cosl(2 * M_PI * j / resolucao), raio * sinl(2 * M_PI * (j + 1) / resolucao), espacamento * i + raio * cosl(2 * M_PI * (j + 1) / resolucao), espacamento * i, altura * valoresnumericos[i] / max, rgb[i]);
